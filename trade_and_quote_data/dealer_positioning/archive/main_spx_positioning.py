...
Orchestrates the complete analysis pipeline from data collection to report generation for SPX
"""
...
# Import our analysis modules (reusing most from SPY version)
from spx_trades_downloader_updated import SPXTradesDownloader
...
def get_current_spx_price():
    """Get current SPX price from Yahoo Finance"""
    try:
        ...
        print(f"Warning: Could not fetch SPX price from Yahoo Finance: {e}")
        fallback_price = 6715.79
        print(f"Using fallback SPX price: ${fallback_price:.2f}")
...
class SPXPositioningPipeline:
    """Main pipeline for SPX dealer positioning analysis"""
    
    def __init__(self, api_key: str, spot_price: float, output_dir: str = "outputs/spx_positioning"):
        ...
        print(f"SPX price: ${self.spot_price:.2f}")
    ...
    def _run_greeks_calculation(self, classified_df: pd.DataFrame) -> tuple:
        ...
        self.calculator = GreeksCalculator(
            ...
            rate=0.05, 
            dividend_yield=0.015  # SPX has dividends
        )
        ...
        # Save Greeks data
        os.makedirs("data/spx_options/greeks", exist_ok=True)
    ...
    def _run_market_analysis(self, aggregated_greeks: pd.DataFrame) -> dict:
        ...
        # Save analysis
        os.makedirs("data/spx_options", exist_ok=True)
    ...
    def _print_summary(self, results: dict, analysis: dict):
        ...
        print(f"   • Charts: greeks_panel.html, gamma_profile.html, volatility_smile.html")
...
def main():
    ...
    parser.add_argument('--spot', type=float, 
                       help='Current SPX spot price (fetched from Yahoo if not provided)')
    parser.add_argument('--api-key', type=str, 
                       help='Polygon API key (or set POLYGON_API_KEY env var)')
    parser.add_argument('--skip-download', action='store_true',
                       help='Skip data download, use existing data')
    parser.add_argument('--output', type=str, default='outputs/spx_positioning',
                       help='Output directory')
    
    args = parser.parse_args()
    
    # Get API key
    api_key = args.api_key or os.getenv('POLYGON_API_KEY') or "placeholder"
    
    # Get current SPX price
    if args.spot:
        current_price = args.spot
        print(f"Using provided SPX price: ${current_price:.2f}")
    else:
        current_price = get_current_spx_price()
    ...
    try:
        # Initialize pipeline
        pipeline = SPXPositioningPipeline(
            api_key=api_key,
        ...
        print(f"\n✅ SPX Analysis completed successfully!")
