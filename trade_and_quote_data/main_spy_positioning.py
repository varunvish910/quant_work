#!/usr/bin/env python3
"""
SPY Weekly Options Dealer Positioning Analysis - Main Entry Point
Orchestrates the complete analysis pipeline from data collection to report generation for SPY
"""
...
# Import our analysis modules (reusing most from SPX version)
...
def get_current_spy_price():
    """Get current SPY price from Yahoo Finance"""
    try:
        ...
        print(f"Warning: Could not fetch SPY price from Yahoo Finance: {e}")
        fallback_price = 669.21
...
class SPYPositioningPipeline:
    """Main pipeline for SPY dealer positioning analysis"""
    
    def __init__(self, api_key: str, spot_price: float, output_dir: str = "outputs/spy_positioning"):
        ...
        self.data_dir = Path("data/spy_options")
        ...
        print(f"Initialized SPY Dealer Positioning Pipeline")
        print(f"Output directory: {self.output_dir}")
        print(f"SPY price: ${self.spot_price:.2f}")
    ...
    def run_full_analysis(self, target_date: str, expiry_dates: list, 
        ...
        print(f"\n{'='*60}")
        print(f"SPY DEALER POSITIONING ANALYSIS")
    ...
    def _run_data_collection(self, target_date: str, expiry_dates: list) -> pd.DataFrame:
        ...
        self.downloader = SPYTradesDownloader(self.api_key)
        
        # Download raw trades
        print(f"Downloading SPY options trades for {target_date}...")
        trades_df = self.downloader.download_trades(target_date, expiry_dates, self.spot_price)
    ...
    def _load_existing_data(self, target_date: str) -> pd.DataFrame:
        ...
        try:
            file_path = f"data/spy_options/trades/{target_date}_enriched_trades.parquet"
    ...
    def _run_trade_classification(self, trades_df: pd.DataFrame, target_date: str) -> pd.DataFrame:
        ...
        output_file = f"data/spy_options/classified/{target_date}_classified_trades.parquet"
    ...
    def _run_greeks_calculation(self, classified_df: pd.DataFrame) -> tuple:
        ...
        self.calculator = GreeksCalculator(
            ...
            rate=0.05, 
            dividend_yield=0.015  # SPY has dividends
        )
        ...
        # Save Greeks data
        os.makedirs("data/spy_options/greeks", exist_ok=True)
        aggregated_greeks.to_parquet("data/spy_options/greeks/aggregated_greeks.parquet", index=False)
        trade_greeks.to_parquet("data/spy_options/greeks/trade_level_greeks.parquet", index=False)
    ...
    def _run_market_analysis(self, aggregated_greeks: pd.DataFrame) -> dict:
        ...
        # Save analysis
        os.makedirs("data/spy_options", exist_ok=True)
        with open("data/spy_options/market_structure_analysis.json", "w") as f:
    ...
    def _print_summary(self, results: dict, analysis: dict):
        ...
        print(f"⚖️  Gamma centroid: ${getattr(key_levels, 'gamma_centroid', 0):.2f}")
        
        if hasattr(key_levels, 'upside_pivot') and key_levels.upside_pivot:
            print(f"⬆️  Upside pivot: ${key_levels.upside_pivot:.2f}")
        
        if hasattr(key_levels, 'downside_pivot') and key_levels.downside_pivot:
            print(f"⬇️  Downside pivot: ${key_levels.downside_pivot:.2f}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='SPY Weekly Options Dealer Positioning Analysis')
    parser.add_argument('--date', type=str, default=datetime.now().strftime("%Y-%m-%d"), 
                       help='Target analysis date (YYYY-MM-DD)')
    parser.add_argument('--expiry', type=str, nargs='+', 
                       help='Option expiry dates (YYYY-MM-DD), defaults to next Friday')
    parser.add_argument('--spot', type=float, 
                       help='Current SPY spot price (fetched from Yahoo if not provided)')
    parser.add_argument('--api-key', type=str, 
                       help='Polygon API key (or set POLYGON_API_KEY env var)')
    parser.add_argument('--skip-download', action='store_true',
                       help='Skip data download, use existing data')
    parser.add_argument('--output', type=str, default='outputs/spy_positioning',
    ...
    if args.spot:
        current_price = args.spot
        print(f"Using provided SPY price: ${current_price:.2f}")
    else:
    ...
    if args.expiry:
        ...
        expiry_dates = [get_next_weekly_expiry()]
        print(f"Using next weekly expiry: {expiry_dates[0]}")
    ...
    try:
        ...
        print(f"\n✅ SPY Analysis completed successfully!")
